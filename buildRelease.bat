...
IF /I "%version%" NEQ "" (
  ...
  Powershell.exe -executionpolicy remotesigned -File SetVersion.ps1 "./pkg/MobSwitcher/tools/VERIFICATION.txt" %version%
)
dotnet restore
...
del .\pkg\MobSwitcher\tools\MobSwitcher-Installer.wixpdb
Powershell.exe -executionpolicy remotesigned -File SetFileHash.ps1
